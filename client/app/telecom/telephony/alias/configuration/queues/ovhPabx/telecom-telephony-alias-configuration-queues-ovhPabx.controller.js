angular.module("managerApp").controller("TelecomTelephonyAliasConfigurationQueuesOvhPabxCtrl", function ($stateParams, $q, $translate, $timeout, $uibModal, Telephony, Toast, ToastError) {
    "use strict";
    ...
    self.fetchSounds = function () {
        return Telephony.OvhPabx().Sound().Lexi().query({
            ...
            return $q.all(_.map(ids, function (id) {
                return Telephony.OvhPabx().Sound().Lexi().get({
                    billingAccount: $stateParams.billingAccount,
                    serviceName: $stateParams.serviceName,
                    soundId: id
                }).$promise;
    ...
    self.fetchAgents = function () {
        return Telephony.OvhPabx().Hunting().Agent().Lexi().query({
            ...
            return $q.all(_.map(_.chunk(ids, 50), function (chunkIds) {
                return Telephony.OvhPabx().Hunting().Agent().Lexi().getBatch({
                    billingAccount: $stateParams.billingAccount,
                    serviceName: $stateParams.serviceName,
                    agentId: chunkIds
                }).$promise;
    ...
    self.fetchAgentsOfQueue = function (queue) {
        return Telephony.OvhPabx().Hunting().Queue().Agent().Lexi().query({
            billingAccount: $stateParams.billingAccount,
            serviceName: $stateParams.serviceName,
    ...
    self.reorderAgentsOfQueue = function (queue, agents) {
        ...
        Telephony.OvhPabx().Hunting().Queue().Agent().Lexi().resetAllCache();
        return $q.all(_.map(_.chunk(ids, 50), function (chunkIds) {
            return Telephony.OvhPabx().Hunting().Queue().Agent().Lexi().getBatch({
                billingAccount: $stateParams.billingAccount,
                serviceName: $stateParams.serviceName,
                queueId: queue.queueId,
                agentId: chunkIds
    ...
    self.swapAgentsOfQueue = function (queue, fromAgent, toAgent) {
        return Telephony.OvhPabx().Hunting().Queue().Agent().Lexi().change({
            billingAccount: $stateParams.billingAccount,
            serviceName: $stateParams.serviceName,
            queueId: queue.queueId,
    ...
    self.deleteAgentFromQueue = function (queue, toDelete) {
        return Telephony.OvhPabx().Hunting().Queue().Agent().Lexi().remove({
            billingAccount: $stateParams.billingAccount,
            serviceName: $stateParams.serviceName,
            queueId: queue.queueId,
    ...
    self.createQueue = function () {
        ...
        return Telephony.OvhPabx().Hunting().Queue().Lexi().create({
            billingAccount: $stateParams.billingAccount,
            serviceName: $stateParams.serviceName
        }, {
    ...
    self.deleteQueue = function (queue) {
        ...
        return Telephony.OvhPabx().Hunting().Queue().Lexi().remove({
            billingAccount: $stateParams.billingAccount,
            serviceName: $stateParams.serviceName,
            queueId: queue.queueId
        }).$promise.then(function () {
    ...
    self.addAgentToQueue = function (queue) {
        queue.isAdding = true;
        return Telephony.OvhPabx().Hunting().Agent().Lexi().addToQueue({
            billingAccount: $stateParams.billingAccount,
            serviceName: $stateParams.serviceName,
    ...
    self.updateQueue = function (queue) {
        ...
        return Telephony.OvhPabx().Hunting().Queue().Lexi().change({
            billingAccount: $stateParams.billingAccount,
            serviceName: $stateParams.serviceName,
            queueId: queue.queueId
        }, {
