angular.module("managerApp").controller("PackMoveCtrl", function ($scope, $compile, $state, $timeout, $translate, $uibModal, $filter, $q, $stateParams,
    ...
    var taskMovePendingName = "pendingAddressMove";
    ...
    this.calendarConfig = {
        ...
        eventRender: function (slot, element) {
            var classNames = ["ovh-pack-move-event"];
        ...
        eventAfterAllRender: function () {
            if (uiCalendarConfig.calendars && uiCalendarConfig.calendars.mainCalendar &&
                ...
                uiCalendarConfig.calendars.mainCalendar.fullCalendar("gotoDate",
    ...
    */
        ...
        if (!moveData) {
            ...
            if (self.offer.selected.lineNumber) {
                moveData.landline = {
                    ...
                    rio: validator.isRio(self.form.futureLandline.rio, self.form.futureLandline.lineNumber) ? self.form.futureLandline.rio.toUpperCase() : undefined,
                ...
                moveData.creation = {
                    ...
                    meeting: {
                        ...
                        meetingSlot: !self.offer.meetingSlots.fakeMeeting ? self.offer.meetingSlots.slot.data : undefined
                    }
                };
            }
    ...
    this.isFormValid = function () {
        var keepLine = !self.form.futureLandline.keepLineNumber ||
                           validator.isRio(self.form.futureLandline.rio, self.form.futureLandline.lineNumber);
        var ftMeeting = (self.offer.selected && self.offer.meetingSlots) &&
                            (!self.offer.selected.meetingSlots ||
                            (self.offer.selected.meetingSlots && self.offer.meetingSlots.fakeMeeting) ||
                            (self.offer.selected.meetingSlots && !self.offer.meetingSlots.fakeMeeting && self.offer.meetingSlots.slot));
    ...
    function isSlammingLine () {
        ...
        ).then(function (xdslLines) {
            var slammingLines = _.filter(xdslLines, function (xdslLine) {
                return xdslLine.value.status === "slamming";
            });
    ...
    function init () {
        ...
        $scope.$watch("PackMove.offer.selected.portability.eligible", function (value) {
            if (!value) {
                self.form.futureLandline.keepLineNumber = false;
            }
        });
        $scope.$watch("PackMove.packAdress.portability", function (value) {
            if (!value) {
