...
describe("Controller: XdslModemRebootCtrl", function () {
    // load the controller"s module
    beforeEach(angular.mock.module("managerAppMock"));
    var dataTest = readJSON("node_modules/ovh-api-services/src/xdsl/modem/xdsl-modem.service.dt.spec.json");
    ...
    function initNewCtrl (pack) {
        ...
        XdslModemRebootCtrl = $controller("XdslModemRebootCtrl", {
            $scope: $scope,
            $stateParams: { serviceName: pack }
        });
    }
    ...
    describe("- Reboot Modem -", function () {

        beforeEach(function () {
            $httpBackend.whenPOST("/apiv6/xdsl/" + xdslPack + "/modem/reboot").respond(201, null);
            initNewCtrl(xdslPack);
        });

        it("reboot the modem", function () {
            XdslModemRebootCtrl.rebootModem();
            $httpBackend.flush();

            expect(PackXdslModemMediator.capabilities.canReboot).toBe(false);
        });
