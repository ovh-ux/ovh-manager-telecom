angular.module("managerApp").controller("TelephonySchedulerEventsCtrl", function ($filter, telephonyScheduler) {
    ...
    self.getAvailableEndSlots = function () {
        var dateStartToMoment = moment(self.event.dateStart);
    ...
    function parseHourChange (date, hour) {
        var dateToMoment = moment(date);
        var hourToMoment = moment(hour, "HH:mm");
    ...
    self.parseFromHour = function (newHour) {
        if (arguments.length) {
            return (self.event.dateStart = parseHourChange(self.event.dateStart, newHour));
        }
    ...
    self.parseToHour = function (newHour) {
        if (arguments.length) {
            return (self.event.dateEnd = parseHourChange(self.event.dateEnd, newHour));
        }
    ...
    self.convertCategoryToSlot = function (category) {
        return telephonyScheduler.convertCategoryToSlot(self.timeCondition, category || self.event.categories);
    };
    ...
    function getFirstAvailableCategory () {
        return _.find(self.categories, function (category) {
            return !self.scheduler.isEventInExistingRange({
                categories: category,
                dateStart: self.event.dateStart,
                dateEnd: self.event.dateEnd
    ...
    self.$onInit = function () {
        ...
        return telephonyScheduler.getAvailableCategories().then(function (categories) {
            self.categories = _.filter(categories, function (category) {
